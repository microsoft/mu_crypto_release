...
def compare_reports(source_reports, target_reports):
    # compare basic report information
    ...
    for arch in source_reports['sizes_per_arch']:
        log_section(f"Comparing binary sizes for Arch: {arch}:")
        if arch not in target_reports['sizes_per_arch']:
            log_warning(f"Architecture {arch} not found in target report!")
            continue
        for binary in source_reports['sizes_per_arch'][arch]:
            if binary not in target_reports['sizes_per_arch'][arch]:
                ...
                continue
            source_binary_size = source_reports['sizes_per_arch'][arch][binary]
            target_binary_size = target_reports['sizes_per_arch'][arch][binary]
        ...
        for binary in target_reports['sizes_per_arch'][arch]:
            if binary not in source_reports['sizes_per_arch'][arch]:
    ...
    for arch in source_reports['linked_openssllib_per_arch']:
        log_section(f"Comparing linked OpensslLib for Arch: {arch}:")

        for binary in source_reports['linked_openssllib_per_arch'][arch]:
            source_linked_openssllib = source_reports['linked_openssllib_per_arch'][arch][binary]
            target_linked_openssllib = target_reports['linked_openssllib_per_arch'][arch][binary]

            if source_linked_openssllib != target_linked_openssllib:
                log_warning(f"Linked OpensslLib has been changed for {binary} in architecture {arch}! - New {source_linked_openssllib}, Old {target_linked_openssllib}")  # log warning - Change in linked OpensslLib*.inf!

    # compare openssl configuration flags
    for file in source_reports['openssl_libs_flags']:
        log_section(f"Comparing OpensslLib configuration flags for file: {file}:")
        if file not in target_reports['openssl_libs_flags']:
        ...
        source_flags = source_reports['openssl_libs_flags'][file]["OPENSSL_FLAGS"]
        target_flags = target_reports['openssl_libs_flags'][file]["OPENSSL_FLAGS"]
        ...
        source_flags = source_reports['openssl_libs_flags'][file]["OPENSSL_FLAGS_CONFIG"]
        target_flags = target_reports['openssl_libs_flags'][file]["OPENSSL_FLAGS_CONFIG"]
    ...
    for file in target_reports['openssl_libs_flags']:
        if file not in source_reports['openssl_libs_flags']:
