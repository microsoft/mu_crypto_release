/** @file
  OneCryptoLoaderDxeByProtocol.c

  This file contains the implementation of the OneCryptoLoader DXE driver,
  which consumes a pre-installed private constructor protocol and publishes
  the OneCrypto protocol for DXE clients.

  RNG REQUIREMENTS:
  - DXE environment requires EFI_RNG_PROTOCOL for cryptographic operations needing entropy
...
/**
 * @brief Install shared dependencies required by OneCrypto.
 *
 * This function populates the dependency table passed to the crypto
 * constructor entry.
 *
 * @param[out] OneCryptoDepends  Dependency table consumed by the constructor.
 */
VOID
  ...
  )
{
  ...
  //
  // Invoke the constructor entry to initialize the protocol
  //
  Status = ConstructorProtocol->Entry (mOneCryptoDepends, &OneCryptoProtocol, &CryptoSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "OneCryptoLoaderDxe: Constructor entry failed: %r\n", Status));
...
Exit:
  ...
  //
  // The OneCrypto dependencies must persist beyond this driver's initialization.
  // Only free them if an error occurred; otherwise, they remain allocated to support
  // ongoing crypto operations.
  //
