  ...
  )
{
  ...
  //
  // NOTE: OpenSSL 4.0 removed its self-contained BIO_snprintf formatter
  // (_dopr/fmtstr) and now routes BIO_(v)snprintf through this platform
  // vsnprintf (openssl PR #28305).  That formatter substituted "<NULL>"
  // for a NULL %s argument; the platform is now expected to provide
  // C99-conforming behaviour instead.  OpenSSL's own error paths pass NULL
  // to %s in several places (see openssl issues #30402 and #31581), which
  // is undefined behaviour in standard C.  EDK2 AsciiVSPrint is safe here:
  // it prints "<null string>" for a NULL %a/%s pointer rather than
  // dereferencing it.  That NULL guard must be preserved -- do not replace
  // AsciiVSPrint with a formatter that lacks it, or OpenSSL 4.0 error
  // reporting can crash on firmware the same way it did on Solaris.
  //
