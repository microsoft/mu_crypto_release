...
# DO NOT MODIFY
# GENERATED ON: 2025-09-26 08:55:42.261655
...
!endif
!if $(PEI_CRYPTO_SERVICES) IN "ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL"
  ...
  !endif
!else
  !error PEI_CRYPTO_SERVICES must be set to one of ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL.
!endif

!ifndef DXE_CRYPTO_SERVICES
  DEFINE DXE_CRYPTO_SERVICES = NONE
!endif
!if $(DXE_CRYPTO_SERVICES) IN "ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL"
  ...
  !endif
!else
  !error DXE_CRYPTO_SERVICES must be set to one of ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL.
!endif

!ifndef RUNTIMEDXE_CRYPTO_SERVICES
  DEFINE RUNTIMEDXE_CRYPTO_SERVICES = NONE
!endif
!if $(RUNTIMEDXE_CRYPTO_SERVICES) IN "ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL"
  ...
  !endif
!else
  !error RUNTIMEDXE_CRYPTO_SERVICES must be set to one of ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL.
!endif

!ifndef SMM_CRYPTO_SERVICES
  DEFINE SMM_CRYPTO_SERVICES = NONE
!endif
!if $(SMM_CRYPTO_SERVICES) IN "ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL"
  ...
  !endif
!else
  !error SMM_CRYPTO_SERVICES must be set to one of ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL.
!endif

!ifndef STANDALONEMM_MMSUPV_CRYPTO_SERVICES
  DEFINE STANDALONEMM_MMSUPV_CRYPTO_SERVICES = NONE
!endif
!if $(STANDALONEMM_MMSUPV_CRYPTO_SERVICES) IN "ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL"
  ...
  !endif
!else
  !error STANDALONEMM_MMSUPV_CRYPTO_SERVICES must be set to one of ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL.
!endif

!ifndef STANDALONEMM_CRYPTO_SERVICES
  DEFINE STANDALONEMM_CRYPTO_SERVICES = NONE
!endif
!if $(STANDALONEMM_CRYPTO_SERVICES) IN "ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL"
  ...
  !endif
!else
  !error STANDALONEMM_CRYPTO_SERVICES must be set to one of ALL NONE TINY_SHA MINIMAL_SHA_SM3 SMALL_SHA_RSA STANDARD ALL.
!endif
...
!if $(PEI_CRYPTO_SERVICES) == ALL
 !if $(PEI_CRYPTO_ARCH) == X64
  [Components.X64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_Pei_$(TARGET)_X64.inf 
 !endif
 !if $(PEI_CRYPTO_ARCH) == AARCH64
  [Components.AARCH64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_Pei_$(TARGET)_AARCH64.inf 
 !endif
 !if $(PEI_CRYPTO_ARCH) == IA32
  [Components.IA32]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_Pei_$(TARGET)_IA32.inf 
 !endif
...
!if $(DXE_CRYPTO_SERVICES) == ALL
 !if $(DXE_CRYPTO_ARCH) == X64
  [Components.X64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_Dxe_$(TARGET)_X64.inf 
 !endif
 !if $(DXE_CRYPTO_ARCH) == AARCH64
  [Components.AARCH64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_Dxe_$(TARGET)_AARCH64.inf 
 !endif
 !if $(DXE_CRYPTO_ARCH) == IA32
  [Components.IA32]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_Dxe_$(TARGET)_IA32.inf 
 !endif
...
!if $(RUNTIMEDXE_CRYPTO_SERVICES) == ALL
 !if $(RUNTIMEDXE_CRYPTO_ARCH) == X64
  [Components.X64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_RuntimeDxe_$(TARGET)_X64.inf 
 !endif
 !if $(RUNTIMEDXE_CRYPTO_ARCH) == AARCH64
  [Components.AARCH64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_RuntimeDxe_$(TARGET)_AARCH64.inf 
 !endif
 !if $(RUNTIMEDXE_CRYPTO_ARCH) == IA32
  [Components.IA32]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_RuntimeDxe_$(TARGET)_IA32.inf 
 !endif
...
!if $(SMM_CRYPTO_SERVICES) == ALL
 !if $(SMM_CRYPTO_ARCH) == X64
  [Components.X64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_Smm_$(TARGET)_X64.inf 
 !endif
 !if $(SMM_CRYPTO_ARCH) == IA32
  [Components.IA32]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_Smm_$(TARGET)_IA32.inf 
 !endif
...
!if $(STANDALONEMM_MMSUPV_CRYPTO_SERVICES) == ALL
 !if $(STANDALONEMM_MMSUPV_CRYPTO_ARCH) == X64
  [Components.X64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_StandaloneMm_MmSupv_$(TARGET)_X64.inf 
 !endif
...
!if $(STANDALONEMM_CRYPTO_SERVICES) == ALL
 !if $(STANDALONEMM_CRYPTO_ARCH) == X64
  [Components.X64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_StandaloneMm_$(TARGET)_X64.inf 
 !endif
 !if $(STANDALONEMM_CRYPTO_ARCH) == AARCH64
  [Components.AARCH64]
    $(SHARED_CRYPTO_PATH)/Driver/Bin/CryptoDriverBin_ALL_StandaloneMm_$(TARGET)_AARCH64.inf 
 !endif
